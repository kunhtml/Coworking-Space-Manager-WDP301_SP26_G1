...
async function applyBookingPaidTransition(booking, now) {
  ...
  if (!["Pending", "Awaiting_Payment", "Confirmed"].includes(currentStatus)) {
    return;
  }
...
export async function createCounterOrderPayment({
  booking,
  order,
  invoice,
  buyer,
  ...
  const description = createShortDescription(
    // Tạo mô tả ngắn gọn cho thanh toán
    booking.bookingCode,
