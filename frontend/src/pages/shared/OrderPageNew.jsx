...
export default function MenuPage() {
  ...
  return (
    <div className="min-vh-100 bg-light">
        ...
        <Container>
          ...
          <Navbar.Collapse>
            <Nav className="me-auto ms-5">
              ...
              <Nav.Link as={Link} to="/order-table" className="fw-medium text-muted px-3">
                Đặt chỗ
              </Nav.Link>
      ...
      {showSuccess && (
        <Alert variant="success" className="mb-0 text-center border-0 rounded-0">
              ...
              {categories.map((category) => (
                ...
                >
                  ...
                  <Badge
                    ...
                    text={selectedCategory === category.id ? "primary" : "white"}
                    className="ms-2"
                  >
                    {category.count}
                  </Badge>
        ...
        <Container>
          {loading ? (
            <div className="text-center py-5">
              ...
              <p className="text-muted mt-3">Đang tải thực đơn...</p>
            </div>
          ) : (
            <>
              <Row className="g-4">
                {getFilteredItems().map((item) => {
                  const itemId = item._id || item.id;
                  const itemInCart = cart.find((c) => (c._id || c.id) === itemId);

                  // ĐÃ SỬA TẠI ĐÂY: Dùng item.categoryId
                  const itemCatId = item.categoryId?._id || item.categoryId;
                  const categoryName = dbCategories.find(c => c._id === itemCatId)?.name || "Chưa phân loại";

                  return (
                    <Col key={itemId} lg={3} md={4} sm={6}>
                      <Card className="h-100 border-0 shadow-sm rounded-4 overflow-hidden hover-lift">
                        <div
                          ...
                          style={{
                            backgroundColor: item.color || "rgba(99, 102, 241, 0.1)",
                            minHeight: "180px",
                            display: "flex",
                        ...
                        >
                          ...
                          <Badge
                            bg="light"
                            text="primary"
                            className="position-absolute top-0 end-0 m-3 px-2 py-1 small shadow-sm"
                          >
                            {categoryName}
                          </Badge>

                          <div className="menu-icon mb-3" style={{ fontSize: "4rem" }}>
                            <i className={item.icon || "bi-cup-hot"}></i>
                          </div>
                        </div>
                            ...
                            {itemInCart && (
                              <div className="d-flex align-items-center justify-content-center gap-2 p-2 bg-light rounded-3">
                                <Button
                                  ...
                                  onClick={() => updateQuantity(itemId, itemInCart.quantity - 1)}
                                >
              ...
              {!loading && getFilteredItems().length === 0 && (
                <div className="text-center py-5">
                  ...
                  <p className="text-muted">Thử chọn danh mục khác nhé</p>
                </div>
              )}
            </>
          )}
          ...
          {cart.length === 0 ? (
            ...
            <>
              ...
              <Form.Group className="mb-4">
                <Form.Label className="fw-semibold">Ghi chú đơn hàng</Form.Label>
                <Form.Control
                  as="textarea"
                  rows={3}
